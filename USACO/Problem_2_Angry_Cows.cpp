...
void solve()
{
    ...
    f(i, 0, n)
    {
        ...
        {
            debug(blast);
            if (blast.find(bale[l]) == blast.end())
            {
                x = lower_bound(all(bale), bale[l] - step) - bale.begin();
                z = (--upper_bound(all(bale), bale[l] + step)) - bale.begin();
            }
            if (blast.find(bale[r]) == blast.end())
            {
                y = (--upper_bound(all(bale), bale[r] + step)) - bale.begin();
                w = lower_bound(all(bale), bale[r] - step) - bale.begin();
            }
            blast.insert(bale[l]);
            blast.insert(bale[r]);
            x = min(x, w);
