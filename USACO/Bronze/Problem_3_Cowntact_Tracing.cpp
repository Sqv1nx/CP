...
void solve()
{
    ll n, t;
    cin >> n >> t;
    vector<bool> tru_infec(n + 1, false);
    f(i, 1, n + 1)
    {
        char x;
        cin >> x;
    ...
    vector<pair<ll, pll>> tem(t + 1);
    f(i, 1, t + 1)
    {
        cin >> tem[i].ff >> tem[i].ss.ff >> tem[i].ss.ss;
    }
    sort(all(tem));
    vector<bool> posi(n + 1, 0), posk(t + 1, 0);
    f(i, 1, n + 1)
    {
        f(k, 0, t + 1)
        {
            bool pos=true;
            vector<bool> infec(n + 1, false);
            vll cnt(n + 1, 0);
            infec[i] = true;
            for (auto [a, b] : tem)
            {
