...
void solve()
{
    ...
    f(i, 0, ea.size())
    {
        f(j, 0, no.size())
        {
            ...
            if (m != n && m >= 0 && n >= 0)
            {
                ...
                {
                    ea[i].e = min(m, ea[i].e);
                    ea[i].k.insert({m, no[j].i});
                }
                else
                {
                    no[j].e = min(no[j].e, n);
                    no[j].k.insert({n, ea[i].i});
                }
            }
        }
    }
    vector<cow> ofarm = farm;
    f(i, 0, ea.size())
    {
        farm[i] = ea[i];
    }
    ...
    struct cc
    {
        bool operator()(const cow &p, const cow &q)
        {
            return p.e > q.e;
        }
    };
    priority_queue<cow, vector<cow>, cc> pq;
    f(i, 0, n)
    {
        pq.push(farm[i]);
    }
