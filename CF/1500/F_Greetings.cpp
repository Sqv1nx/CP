...
void merge(vpll &a, ll p, ll q, ll r, ll &ans)
{
    ll n1 = q - p + 1, n2 = r - q;
    vpll j(n1 + 1), k(n2 + 1);
    f(i, 0, n1)
...
void mergesort(vpll &a, ll p, ll r, ll &ans)
{
    if (p >= r)
        return;
    ll q = p + (r - p) / 2;
    mergesort(a, p, q, ans);
    mergesort(a, q + 1, r, ans);
    merge(a, p, q, r, ans);
}
void solve()
{
    //thanks to mr nepali mishra for giving this idea after DS exams 
    ll n;
    cin >> n;
    vpll a(n);
    f(i, 0, n)
