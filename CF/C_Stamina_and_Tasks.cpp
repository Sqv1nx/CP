...
void solve()
{
    // solution may look like ai cause i use ai for logic while trying this question
    // first question w proper dp ig
    ll n;
    cin >> n;
    vector<pll> task(n);
    f(i, 0, n)
    ...
    dp[0] = 0.0; // since task reverse then no task wil cause addition of 0 points
    f(i, 1, n + 1)
    {
        dp[i] = dp[i - 1];
        dp[i] = max(dp[i], task[i-1].ff * 1.0 + ((1.0 - (task[i-1].ss) / 100.0) * dp[i - 1]));
    }
...
//reverse to carray the damage across all
