...
void solve()
{
    ...
    {
        ll k;
        //use ai mistake was not consider when k==0;
        if (i > 0)
        {
            k = upper_bound(all(pre), tea[i] + pre[i - 1]) - pre.begin();
            ans[k] += tea[i] - pre[k - 1] + pre[i - 1];
        }
        else
        {
            k = upper_bound(all(pre), tea[i]) - pre.begin();
            if (k > 0)
                ans[k] += tea[i] - pre[k - 1];
            else
                ans[k] += tea[i];
