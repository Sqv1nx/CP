...
void solve()
{
    ...
    f(i, 0, n - 2)
    {
        ll temp = abs(num[i+1] - num[i]) + abs(num[i+1] - num[i + 2]) - abs(num[i + 2] - num[i]);
        problem=max(temp,problem);
    }
    problem=max(problem,abs(num[1]-num[0]));
    problem=max(problem,abs(num[n-1]-num[n-2]));
