...
void solve()
{
    ...
    f(i, 0, m)
    {
        cin >> code[i].ff >> code[i].ss;
    }
    sort(all(chess));
    sort(all(code), [](pll a, pll b)
         { return a.ss < b.ss; });
    debug(chess);
    debug(code);
    ll x = max((ll)0, chess[n - 1].ff - code[0].ss);
    sort(all(code));
    sort(all(chess), [](pll a, pll b)
         { return a.ss < b.ss; });
    debug(chess);
    debug(code);
    x = max(x, code[m - 1].ff - chess[0].ss);
    cout<<x<<'\n';
}
