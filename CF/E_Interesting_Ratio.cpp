...
void sieve()
{
    fill(is_prime, is_prime + nb, true);
    is_prime[0] = is_prime[1] = false;
    for (int p = 2; p * p < nb; p++)
    {
        if (is_prime[p])
        {
            for (int i = p * p; i < nb; i += p)
                is_prime[i] = false;
        }
    }
    for (int p = 2; p < nb; p++)
...
void solve()
{
    ...
    {
        ll t = n / k;
        ll p=((upper_bound(all(primes), t)) - primes.begin());
        debug(p);
        ans += ((upper_bound(all(primes), t)) - primes.begin());
        k++;
    }
    cout << ans << '\n';
}
