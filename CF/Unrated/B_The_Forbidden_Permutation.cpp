...
vector<T> compress(vector<T>& a) {
    ...
    for (auto& x : a) x = lower_bound(all(vals), x) - vals.begin() + 1; // 1-indexed
    return vals;
}
template <typename T>
...
struct Fenwick {
    ...
    void add(int i, ll delta) { for (; i <= n; i += i & (-i)) tree[i] += delta; }
...
struct SegTree {
    ...
    ll query(int l, int r) { // inclusive, 0-indexed
...
struct LazySegTree {
    ...
    ll query(int node, int start, int end, int l, int r) {
        push(node, start, end);
        if (start > end || start > r || end < l) return 0; // 0 for sum, -INF/INF for max/min
...
struct StringHash {
    ...
    pll get(int l, int r) { // inclusive, 0-indexed substring hash
