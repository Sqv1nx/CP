...
void solve()
{
    ...
    f(i, 0, n)
    {
        ...
        y = y / p;
        x = (x - 1) / p;
        cor[i] = ((y - x) * 1.0) / (1.0 * pos[i]);
        wor[i] = 1 - cor[i];
    }
    vector<double> earnings(n, 0.0);
    double expect = 0.0;
    ll deno = 1;
    f(i, 0, n)
    {
        ...
        if (i == 0)
        {
            earnings[0] = 2000.00 * (cor[0] + cor[n - 1] * wor[0] * cor[1]) + 1000.00 * (cor[n - 1] * wor[0] * wor[1] + wor[n - 1] * wor[0] * cor[1]);
        }
        else if (i == n - 1)
        {
            earnings[n - 1] = 2000.00 * (cor[n - 1] + cor[n - 2] * wor[n - 1] * cor[0]) + 1000.00 * (cor[n - 2] * wor[n - 1] * wor[0] + wor[n - 2] * wor[n - 1] * cor[0]);
        }
        else
        {
            earnings[i] = 2000.00 * (cor[i] + cor[i - 1] * wor[i] * cor[i + 1]) + 1000.00 * (cor[i - 1] * wor[i] * wor[i + 1] + wor[i - 1] * wor[i] * cor[i + 1]);
        }
    }
    for (auto x : earnings)
    {
    ...
    cout << fixed << setprecision(10) << expect << '\n';
}
