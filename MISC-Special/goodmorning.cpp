...
void bt(vector<vll> &grid, str s, ll &ans, ll &n, ll y, ll x)
{
    if (s.size() <= 3 && !s.empty())
    {
        ll a = stoll(s);
        if (abs(n - a) < abs(n - ans))
        {
            ans = a;
        }
        if (s.size() == 3)
    ...
    if (y < 3 && grid[y][x] != 9)
    {
        bt(grid, s, ans, n, y + 1, x);
    }
    if (x < 2)
    {
        bt(grid, s, ans, n, y, x + 1);
    }
...
void solve()
{
    ll n;
    cin >> n;
    vector<vll> grid = {{1, 2, 3}, {4, 5, 6}, {7, 8, 9}, {0, 0, 0}};
    ll ans = 0;
    str s = "";
    bt(grid, s, ans, n, 0, 0);
    cout << ans << '\n';
