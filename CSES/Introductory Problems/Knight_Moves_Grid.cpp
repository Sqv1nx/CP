...
void solve()
{
    ...
    while (!q.empty())
    {
        auto [i, j] = q.front();
        q.pop();
        if (i - 2 >= 0 && j + 1 < n && num[i - 2][j + 1] == -1)
        {
            num[i - 2][j + 1] = num[i][j] + 1;
            q.push({i - 2, j + 1});
        }
        if (i - 1 >= 0 && j + 2 < n && num[i - 1][j + 2] == -1)
        {
            num[i - 1][j + 2] = num[i][j] + 1;
            q.push({i - 1, j + 2});
        }
        if (i - 1 >= 0 && j - 2 >= 0 && num[i - 1][j - 2] == -1)
        {
            num[i - 1][j - 2] = num[i][j] + 1;
            q.push({i - 1, j - 2});
        }
        if (i - 2 >= 0 && j - 1 >= 0 && num[i - 2][j - 1] == -1)
        {
            num[i - 2][j - 1] = num[i][j] + 1;
            q.push({i - 2, j - 1});
        }
        if (i + 2 < n && j - 1 >= 0 && num[i + 2][j - 1] == -1)
        {
            num[i + 2][j - 1] = num[i][j] + 1;
            q.push({i + 2, j - 1});
        }
        if (i + 1 < n && j - 2 >= 0 && num[i + 1][j - 2] == -1)
        {
            num[i + 1][j - 2] = num[i][j] + 1;
            q.push({i + 1, j - 2});
        }
        if (i + 1 < n && j + 2 < n && num[i + 1][j + 2] == -1)
        {
            num[i + 1][j + 2] = num[i][j] + 1;
            q.push({i + 1, j + 2});
        }
        if (i + 2 < n && j + 1 < n && num[i + 2][j + 1] == -1)
        {
            num[i + 2][j + 1] = num[i][j] + 1;
            q.push({i + 2, j + 1});
        }
