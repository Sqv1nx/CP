...
// bitmasking idea is ai gnerated , its very good
inline int cellBit(ll y, ll x) { return y * 7 + x; }

void bt(str &s, ll idx, ll &cnt, ll y, ll x, uint64_t &visited)
{
    ...
    if (idx == 48)
        return;
    bool up = (y == 0) || (visited & (1ULL << cellBit(y - 1, x)));
    bool down = (y == 6) || (visited & (1ULL << cellBit(y + 1, x)));
    bool left = (x == 0) || (visited & (1ULL << cellBit(y, x - 1)));
    bool right = (x == 6) || (visited & (1ULL << cellBit(y, x + 1)));

    if (up && down && !left && !right)
        return;
    if (left && right && !up && !down)
        return;
    visited |= (1ULL << cellBit(y, x));

    if (s[idx] == '?' || s[idx] == 'U')
    {
        if (!up)
            bt(s, idx + 1, cnt, y - 1, x, visited);
    }
    if (s[idx] == '?' || s[idx] == 'D')
    {
        if (!down)
